...
import {
  Heart, Bookmark, Share2, Download, MoreHorizontal,
  Eye, Lock, ShieldCheck, Repeat2, Bot, Tag
} from 'lucide-react'
...
export default function PinCard({ pin, showCreator = true }: PinCardProps) {
  ...
  return (
    <>
      ...
      >
        {/* Media container */}
        <Link to={`/pin/${pin.id}`} className="block relative overflow-hidden rounded-2xl bg-virens-gray">
          <div style={aspectStyle} className="relative">
            ...
            {/* Protected overlay */}
            {isProtected && <ProtectedOverlay watermark={pin.hasVisibleWatermark ? pin.creator.username : undefined} />}
                ...
                >
                  ...
                  <div className="absolute right-3 top-3 flex flex-col gap-2">
                    ...
                    <ActionButton
                      ...
                      onClick={(e) => { e.preventDefault(); repost() }}
                    />
                    {canDownload && (
                      <ActionButton
                        icon={Download}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-3">
                    ...
                    {pin.tags.length > 0 && (
                      <div className="flex gap-1 flex-wrap">
                        {pin.tags.slice(0, 3).map((tag) => (
                          <span key={tag} className="text-[10px] text-virens-white-muted bg-black/30 px-2 py-0.5 rounded-full">
                            #{tag}
                          </span>
                        ))}
                      </div>
                    ...
                    {pin.isForSale && (
                      <div className="mt-2 flex items-center gap-2">
                        {pin.originalPrice && pin.salePrice && pin.originalPrice > pin.salePrice ? (
                          <>
                            ...
                            <span className="badge-green text-[10px]">
                              Save {Math.round(((pin.originalPrice - pin.salePrice) / pin.originalPrice) * 100)}%
                            </span>
                          </>
                        ) : (
        ...
        {showCreator && (
          <div className="flex items-center gap-2 px-1 pt-2 pb-1">
            <Link to={`/${pin.creator.username}`} className="flex items-center gap-2 flex-1 min-w-0 group/creator">
              {pin.creator.avatar ? (
                <img
            ...
            <div className="flex items-center gap-2 text-virens-white-muted">
              <span className="text-[10px]">{formatNumber(pin.viewsCount)} views</span>
            </div>
          </div>
        )}
      </motion.div>
      ...
      <AnimatePresence>
        {showDownloadModal && (
          <DownloadModal pin={pin} onClose={() => setShowDownloadModal(false)} />
        )}
      </AnimatePresence>
    </>
  )
...
function ActionButton({
  ...
  return (
    ...
    >
      ...
      />
      {count !== undefined && (
        <span className="text-[9px] font-medium leading-none">{formatNumber(count)}</span>
      )}
    </button>
  )
}
